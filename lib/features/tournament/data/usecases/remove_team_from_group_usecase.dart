import 'package:cricklo/core/errors/failure.dart';
import 'package:cricklo/core/usecase/usecase.dart';
import 'package:cricklo/features/notifications/domain/entities/invite_response_response_entity.dart';
import 'package:cricklo/features/tournament/data/entities/add_team_to_group_usecase_entity.dart';
import 'package:cricklo/features/tournament/domain/repo/tournament_repo.dart';
import 'package:dartz/dartz.dart';

class RemoveTeamFromGroupUsecase
    extends UseCase<InviteResponseResponseEntity, AddTeamToGroupUsecaseEntity> {
  final TournamentRepo _tournamentRepo;

  RemoveTeamFromGroupUsecase(this._tournamentRepo);

  @override
  Future<Either<Failure, InviteResponseResponseEntity>> call(
    AddTeamToGroupUsecaseEntity entity,
  ) {
    return _tournamentRepo.addToGroup(entity.tournamentId, {
      "teamId": entity.teamId,
      "groupName": entity.groupName,
    });
  }
}
